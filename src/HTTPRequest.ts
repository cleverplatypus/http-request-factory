...
export class HTTPRequest {
  ...
  async execute(): Promise<any> {
    ...
    for (const interceptor of this.config
      .requestInterceptors || []) {
      const interceptorResponse = await interceptor(this, {
      ...
      if (this.config.responseBodyTransformer) {
        return this.config.responseBodyTransformer(interceptorResponse, this);
      }
